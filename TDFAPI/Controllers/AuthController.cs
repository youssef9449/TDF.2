...
namespace TDFAPI.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<TokenResponse>>> Login([FromBody] LoginRequestDto request)
        {
            ...
            if (userAuth?.IsLocked == true && userAuth.LockoutEnd.HasValue &&
                userAuth.LockoutEnd.Value > DateTime.UtcNow)
            {
                var remainingLockoutTime = Math.Ceiling((userAuth.LockoutEnd.Value - DateTime.UtcNow).TotalMinutes);
                _logger.LogWarning("Failed login attempt for locked account: {Username} from IP {IpAddress}",
                    request.Username, ipAddress);
                return Unauthorized(ApiResponse<TokenResponse>.ErrorResponse(
                    $"Account is temporarily locked. Please try again in {remainingLockoutTime} minute(s)"));
            }
        ...
        public async Task<ActionResult<ApiResponse<RegisterResponseDto>>> Register([FromBody] RegisterRequestDto request)
        {
            ...
            if (newUser == null)
            {
                return StatusCode(500, ApiResponse<RegisterResponseDto>.ErrorResponse(
