    ...
    public class NetworkMonitorService
    {
        ...
        private async Task TriggerReconnectionAttempts()
        {
            try
            {
                ...
                // Test API connectivity using fallback implementation
                bool apiAvailable = false;
                try
                {
                    apiAvailable = await ApiConfig.TestApiConnectivityAsync(null);
                }
                catch (Exception ex)
                {
        ...
        public async Task<(bool IsConnected, bool IsApiReachable)> TestConnectivityAsync()
        {
            ...
            if (isConnected)
            {
                try
                {
                    // Use fallback implementation for background connectivity testing
                    isApiReachable = await ApiConfig.TestApiConnectivityAsync(null);
                }
                catch (Exception ex)
                {
                    DebugService.LogError("NetworkMonitor", $"API connectivity test failed: {ex.Message}");
